...
{
  id:"g113", cat:"gsap", name:"גריד שמסתדר מחדש בגלילה", tech:"GSAP · ScrollTrigger · Flip", status:"ממתין",
  desc:"אותם שישה כרטיסים, שלוש פריסות: רשימה צפופה, גריד שווה, ובנטו עם כרטיס גיבור. בכל שלב גלילה הכרטיסים נוסעים למקומם החדש במקום לקפוץ.",
  when:"תיק עבודות, קטלוג, סקשן צוות. כשהתוכן קבוע והפריסה היא הסיפור. שלושה מצבים לכל היותר.",
  note:"Flip.getState לפני החלפת מחלקת הפריסה ו-Flip.from אחריה, עם absolute:false כדי לא לרוקן את המכל (המלכודת של g23 ו-b34). המעבר נורה על כניסת שלב, לא בסקראב, כי Flip לא אוהב scrub. במובייל שלוש הפריסות הן עמודה, שתי עמודות, ובנטו של 2.",
  libs:["gsap","ScrollTrigger","Flip"],
  css:`.rg{display:grid;grid-template-columns:1fr 1fr;gap:var(--gap);align-items:start;max-width:1100px;margin-inline:auto}
.rg-pin{position:sticky;top:0;height:100vh;display:grid;align-content:center}
.rg-grid{display:grid;gap:12px;width:100%}
.rg-grid.l1{grid-template-columns:1fr}
.rg-grid.l1 .rg-c{height:52px;flex-direction:row;align-items:center;justify-content:space-between}
.rg-grid.l2{grid-template-columns:repeat(3,1fr)}
.rg-grid.l2 .rg-c{aspect-ratio:1}
.rg-grid.l3{grid-template-columns:repeat(3,1fr);grid-auto-rows:110px}
.rg-grid.l3 .rg-c:first-child{grid-column:span 2;grid-row:span 2}
.rg-c{display:flex;flex-direction:column;justify-content:flex-end;gap:4px;padding:14px;border-radius:14px;color:#fff;font-weight:700;font-size:14px;overflow:hidden}
.rg-c small{font-weight:500;opacity:.8;font-size:12px}
.rg-steps{display:flex;flex-direction:column;gap:40vh;padding-block:36vh}
.rg-step h3{margin:0 0 8px;font-size:clamp(22px,2.6vw,36px)}
.rg-step p{margin:0;color:var(--muted);line-height:1.7;max-width:36ch}
@media(max-width:767px){.rg{grid-template-columns:1fr}.rg-pin{height:52vh;top:0;z-index:2;background:var(--bg)}.rg-grid.l2{grid-template-columns:1fr 1fr}.rg-grid.l3{grid-template-columns:1fr 1fr;grid-auto-rows:70px}.rg-steps{gap:24vh;padding-block:6vh 30vh}}`,
  html:`<div class="stage tight"><div class="rg">
  <div class="rg-pin"><div class="rg-grid l1">
    <div class="rg-c ph-a" style="background:linear-gradient(160deg,#3b5bdb,#748ffc)">קליניקה<small>אתר תדמית</small></div>
    <div class="rg-c" style="background:linear-gradient(160deg,#5f3dc4,#9775fa)">חנות<small>איקומרס</small></div>
    <div class="rg-c" style="background:linear-gradient(160deg,#0b7285,#3bc9db)">משרד<small>שלוש שפות</small></div>
    <div class="rg-c" style="background:linear-gradient(160deg,#2b8a3e,#69db7c)">סטודיו<small>תיק עבודות</small></div>
    <div class="rg-c" style="background:linear-gradient(160deg,#e8590c,#ffa94d)">קורס<small>דף נחיתה</small></div>
    <div class="rg-c" style="background:linear-gradient(160deg,#c2255c,#f783ac)">מערכת<small>CRM</small></div>
  </div></div>
  <div class="rg-steps">
    <div class="rg-step"><h3>רשימה</h3><p>כשרוצים לסרוק מהר: שם, סוג, וזהו.</p></div>
    <div class="rg-step"><h3>גריד</h3><p>כשכולם שווים ורוצים לראות את הכמות.</p></div>
    <div class="rg-step"><h3>בנטו</h3><p>כשיש פרויקט אחד שמוביל והשאר תומכים.</p></div>
  </div>
</div></div>`,
  js:`(function(){
  const grid=document.querySelector(".rg-grid"),steps=gsap.utils.toArray(".rg-step"),layouts=["l1","l2","l3"];
  const reduce=matchMedia("(prefers-reduced-motion: reduce)").matches;
  let cur=0;
  function go(k){if(k===cur)return;cur=k;
    const state=Flip.getState(".rg-c");
    grid.classList.remove(...layouts);grid.classList.add(layouts[k]);
    if(reduce)return;
    // absolute:false: המכל שומר על גובהו והעמוד לא קופץ (המלכודת של g23 ו-b34)
    Flip.from(state,{duration:.8,ease:"power3.inOut",stagger:.03,absolute:false});
  }
  steps.forEach((s,i)=>ScrollTrigger.create({trigger:s,start:"top 60%",end:"bottom 40%",onEnter:()=>go(i),onEnterBack:()=>go(i)}));
})();`
},
{
  id:"g114", cat:"gsap", name:"חתימה שנכתבת בגלילה", tech:"GSAP · ScrollTrigger · DrawSVG", status:"ממתין",
  desc:"קו חתימה או שם בכתב יד שנכתב מההתחלה לסוף בקצב הגלילה, ומתחתיו שם ותפקיד שמופיעים כשהקו מסתיים. חתימה חיה בסוף סיפור אישי.",
  when:"עמוד אודות, מכתב מהמייסד, סיום עדות ארוכה. פעם אחת בעמוד, במקום שבו הקול האישי מדבר.",
  note:"DrawSVG על path של החתימה בסקראב (בפרויקט אמיתי: החתימה של הלקוח כ-SVG עם stroke, לא fill). קצה הקו עגול כדי שיראה כמו עט. במובייל אותו SVG ברוחב מלא.",
  libs:["gsap","ScrollTrigger","DrawSVGPlugin"],
  css:`.sg{max-width:min(640px,92vw);margin-inline:auto;padding-block:16vh 30vh;text-align:center}
.sg p{font-size:clamp(18px,1.8vw,24px);line-height:1.7;color:var(--muted);margin:0 0 30px}
.sg svg{width:min(360px,80vw);height:auto;overflow:visible;display:block;margin-inline:auto}
.sg path{fill:none;stroke:var(--ink);stroke-width:3;stroke-linecap:round;stroke-linejoin:round}
.sg-by{margin-top:10px;opacity:0;translate:0 8px}
.sg-by b{display:block;font-size:17px}
.sg-by span{font-size:14px;color:var(--muted)}`,
  html:`<div class="sg">
  <p>הבטחתי לעצמי שכל פרויקט יסתיים באתר שאני הייתי רוצה לקבל. עד היום זה הסטנדרט.</p>
  <svg viewBox="0 0 360 120" aria-label="חתימה"><path class="sg-path" d="M20 80 C 40 20, 60 20, 70 70 S 90 110, 110 60 C 120 40, 130 40, 135 65 C 140 90, 150 95, 165 55 C 175 30, 185 35, 190 60 C 195 85, 205 90, 220 60 C 235 30, 250 40, 255 70 C 260 95, 275 95, 290 55 C 300 30, 320 40, 340 60"/></svg>
  <div class="sg-by"><b>ליאב מצרי</b><span>מייסד</span></div>
</div>`,
  js:`(function(){
  const path=document.querySelector(".sg-path");
  if(matchMedia("(prefers-reduced-motion: reduce)").matches){gsap.set(".sg-by",{opacity:1,translate:"0 0"});return;}
  gsap.set(path,{drawSVG:"0%"});
  gsap.timeline({scrollTrigger:{trigger:".sg svg",start:"top 80%",end:"top 30%",scrub:.5}})
    .to(path,{drawSVG:"100%",ease:"none",duration:1})
    .to(".sg-by",{opacity:1,translate:"0 0",duration:.25},.85);
})();`
},
{
