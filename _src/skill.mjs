...
export function compositionsMarkdown(entries) {
  ...
  out.push(`6. **ציר הראש לכל קומפוזיציה** (ליאב, 24.9.2026; החוק ב-\`engine/spacing-and-axes.md\` 5, נאכף ב-\`axis.mjs\` וב-\`project-check\`): הראש הולך אחרי הגוף, והטלפון מכריע מחדש ולא יורש מהדסקטופ.`);
  out.push("");
  out.push("| קומפוזיציה | ראש בדסקטופ | ראש בטלפון |");
  out.push("|---|---|---|");
  out.push(`| C1 חצי-חצי, C2 חופף | התחלה, הכותרת בעמודת הטקסט | ממורכז כשמתחת לראש רק ויז'ואל וכפתור; התחלה כשיש טקסט רץ |`);
  out.push(`| C3 זיגזג | ממורכז מעל השורות | התחלה (שורות של טקסט ותמונה) |`);
  out.push(`| C4 גריד כרטיסים, C5 בנטו | ממורכז, וגם הסינון, המונה והכפתור | ממורכז, פנים הכרטיס מיושר |`);
  out.push(`| C6 ציר ממוספר | ממורכז, הציר בלוק ממורכז | ממורכז, הטקסט בצעדים מיושר |`);
  out.push(`| C7 מילה ענקית, C10 פס רץ, C14 מונולוג, C16 שער כפול | ממורכז | ממורכז |`);
  out.push(`| C8 עריכתי, C11 מפוצל דביק | התחלה | התחלה, data-axis="start" |`);
  out.push(`| C9 שכבות צפות | לפי הוויז'ואל: ממורכז כשהוא במרכז, התחלה כשהוא בצד | ממורכז |`);
  out.push(`| C12 מסדרון אופקי | התחלה, בצד שבו המסדרון מתחיל | ממורכז (הגוף הוא החלקה בלי טקסט רץ) |`);
  out.push(`| C13 פסיפס | התחלה מעל פסיפס שנוטה לצד, ממורכז מעל פסיפס מאוזן | ממורכז |`);
  out.push(`| C15 דו-קומה | התחלה, מעל הגיבור | ממורכז כשהגיבור ויז'ואל, התחלה כשהוא טקסט |`);
  out.push("");
...
export function stylesMarkdown(entries) {
  ...
  out.push(`1. **עורות מלאים**, נכרו מ-ground truth, מוכנים לפרודקשן: \`storeos-quiet\` (מינימליזם עריכתי), \`soft-modern\` (Soft UI מודרני), \`dark-luxury\` (כהה יוקרתי, נכרה מסברדלוב, מגן מצדה ו-/start). כשיש עור מלא שמתאים, הוא עדיף תמיד.`);
  ...
  out.push(`**פונטים (ליאב, 22.9.2026):** הפונטים שבמתכונים ובדמואים (Suez One, Frank Ruhl Libre, Rubik, Secular One, Varela Round, Karantina, Inter ועוד) הם פונטי גוגל שמשמשים **רק כמחליף ציבורי בדמו**, כי המאגר ציבורי ואסור להטמיע בו פונטים ברישיון. **בפרויקט אמיתי לא משתמשים בהם**: הצימוד נבחר מ-\`library/fonts.md\` לפי אופי העסק, ולכל אופי שם רשימת השפות שמתאימות לו.`);
